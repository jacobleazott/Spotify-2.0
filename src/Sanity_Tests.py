...
#   - Check that all supporting artists have ALL tracks in their '__', ie a song by powfu and promoting sounds should be in both __ playlists
#
...
class SanityTest:
    ...
    # List of tracks to disregard for our comparisons, this currently includes our "shuffle macro" as well as our "Soundtracks" playlists tracks
    ...
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    DESCRIPTION: Inits our spotify api, grabs all user and playlist data we will need for the sanity tests
    INPUT: NA
    OUTPUT: NA
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def __init__(self):
    ...
    def _gather_playlist_data(self):
        for playlist in self.user_playlists:
            if playlist["name"].startswith('__'):
                ...
                self.individual_artist_playlists.append((playlist['name'][2:], self._get_playlist_tracks_info(playlist["id"])))
    ...
    def _find_duplicates(self, tracks):
        ...
        for track in tracks:
            ...
            if track[0] != None and track[0] in checked_list and track not in duplicates and track[0] != gsh.SHUFFLE_MACRO_ID:
    ...
    INPUT: key_track_list - list of dictionary tracks to compare against
           ...
           disregard_tracks (optional) - bool whether we want to disregard 'track_list_to_disregard'
    ...
    def sanity_diffs_in_major_playlist_sets(self):
        ...
        # We skip the disregard tracks when comparing against the '__' playlists because we don't expect them to be in there
        ...
        res_list.append(("TRACKS IN MASTER PLAYLISTS BUT NOT '__'", 
                         self._compare_track_lists(master_track_list, individual_artists_track_list, disregard_tracks=True)))
        ...
        res_list.append(("TRACKS IN YEARS PLAYLISTS BUT NOT '__'", 
                         self._compare_track_lists(years_track_list, individual_artists_track_list, disregard_tracks=True)))
    ...
    def sanity_in_progress_artists(self):
        res_list = []
        for playlist in self.user_playlists:
            if playlist['name'].startswith("__") and not any(playlist['name'][2:] == artist["name"] for artist in self.user_followed_artists):
    ...
    def sanity_duplicates(self):
        ...
        tmp_years_dupe_list = self._find_duplicates([track for playlist in self.years_playlists for track in playlist[1]])
    ...
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    DESCRIPTION: Goes through all combined '__' tracks and finds any that should be in more playlists. ie. tracks that have
                 multiple contributing artists we follow should be in every single one of those '__' playlists
    INPUT: NA
                    ...
                    for check_playlist in self.individual_artist_playlists:
                        if check_playlist[0] in artist_list:
                            if track[0] not in [tmp_track[0] for tmp_track in check_playlist[1]]:
                                res_list.append(f"{track[1]} - {track[2][0]['name']} = IS MISSING FROM = __{check_playlist[0]}")
